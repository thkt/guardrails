...
- **カスタムルール**: 外部リンターがカバーしないセキュリティパターン（JS/TS + Rust）
...
| ルール        | 重大度 | 説明                                                    | 無効化する場面                            |
| ------------- | ------ | ------------------------------------------------------- | ----------------------------------------- |
| `unsafeUsage` | Medium | unsafe ブロック/fn/impl（テストコードを除外）           | FFI が多いクレート、低レベルライブラリ    |
| `unwrapUsage` | Medium | 過度な .unwrap()（ファイルあたり 3 以上、テストを除外） | CLI ツール、プロトタイプ                  |
| `todoMacro`   | Medium | プロダクションコード内の todo!()/unimplemented!()       | 初期プロトタイピング                      |
| `cargoLock`   | Medium | ライブラリクレート内の Cargo.lock                       | バイナリクレート（Cargo.lock は想定通り） |
